...
written directly into shared_ram (see shared_ram.v) with plain SW
instructions to DMEM 0x00-0x7F - that range IS instruction memory
0x80-0xFF (the same 128 physical bytes, addressed from two different
ports), so no separate write-control registers are needed; a received
byte is immediately executable at IMEM[0x80 + its DMEM address] as
...
resolves to external flash instead of shared_ram, then jumps there -
...
is required, not optional. boot_rom (0x00-0x7F) is unconditionally
boot_rom regardless of FLASH_MODE - only 0x80-0xFF's source (shared_ram
vs flash) depends on it - see tt_um_agila8.v's header for the full
reasoning, and the timeout-path comment below for the bug this fixes.

Leftover register state on the timeout path (worth knowing, not a
bug): the WAIT_START loop leaves r1=31 and r6=31 (both driven by the
timeout-counter/threshold comparison) and r4=0 (the last masked
GPIO_IN read, which reads 0 precisely because timeout means START was
never seen) in the register file when control hands off to flash - r2,
r3, r5, and r7 are never touched on this path and stay 0, but that's
incidental to this specific loop structure, not a guarantee. Any flash
program that reaches this way in should explicitly initialize whatever
registers it actually cares about rather than assume a clean start -
r1/r6 in particular will NOT be 0.
"""
...
# Registers:
# r1 = timeout counter        r2 = bit counter (RECV_BYTE only)
# r3 = received byte accum    r4 = GPIO_IN scratch
# r5 = length remaining       r6 = DMEM write pointer (BYTE_LOOP) / mask
#                                   constants (WAIT_START, before
#                                   START_SEEN - see bug-fix note below)
# r7 = link / jump-target scratch (JAL always uses r7 - see bug-fix note below)
...
# *** BUG FIX (see chat): tt_um_agila8.v's boot_rom_hit is now
# unconditional for imem_addr<0x80 (fixed separately - see
# sim_full_top.py's header for why: the old `&&!flash_mode_r` gating
# made boot_rom's OWN code, including this very JALR, disappear the
# instant FLASH_MODE was set, since the fetch of THIS instruction's own
# address would then incorrectly route to flash instead of boot_rom).
# That fix means flash_mode now only changes what backs 0x80-0xFF, not
# 0x00-0x7F - so JALR must target 0x0080 here, not 0x0000 (0x0000 would
# still resolve to boot_rom's own WAIT_START, looping forever exactly
# like the original bug, just via a different address). Same
# load-then-JALR pattern already proven correct in LOAD_DONE below.
a.ADDI(7, 0, 2)
...
# --- Write pointer into shared_ram, addressed from the DMEM side
#     (0x00-0x7F). shared_ram.v merges DMEM and IRAM into the SAME 128
#     physical bytes - DMEM[N] IS IMEM[0x80+N], not a copy of it - so
#     plain SW instructions here are all loading needs; no separate
#     write-control registers like the old IMEM_WADDR/IMEM_WDATA. ---
a.ADDI(6, 0, 0)                  # r6 = 0 (DMEM write pointer)
# *** BUG FIX (see chat): this pointer MUST NOT be r2 - RECV_BYTE uses
# r2 internally as its own bit counter (see RECV_BYTE below), and is
# called every single BYTE_LOOP iteration BEFORE the SW that uses this
# pointer. With r2 as the pointer, every write silently landed at
# DMEM[8] instead of the intended incrementing sequence, because
# RECV_BYTE always leaves r2=8 right before returning (the bit counter
# naturally ends there after receiving a full byte) - confirmed by
# tracing every SW in a full run: all 8 writes showed dmem_addr=0x08,
# each simply overwriting the last, while mem[0..7] (where the loaded
# program actually needs to live) stayed permanently unwritten (X) -
# exactly the "mem[] entirely unwritten" symptom this was chasing. r6
# is free for this: RECV_BYTE never touches it, and nothing else needs
# it live across a RECV_BYTE call in this program.

a.label("BYTE_LOOP")
a.BEQ(5, 0, "LOAD_DONE")        # while (r5 != 0)
a.JAL(7, "RECV_BYTE")           # r3 = received byte
a.SW(3, 6, 0)                    # DMEM[r6] = r3  (== IMEM[0x80 + r6])
a.ADDI(6, 6, 1)                  # r6++
a.ADDI(5, 5, -1)
a.JAL(0, "BYTE_LOOP")
...
a.label("LOAD_DONE")
a.ADDI(7, 0, 2)
a.SLL(7, 7, 6)                   # r7 = 2 << 6 = 0x80 (IRAM base - same
                                   # physical bytes just written via DMEM)
...
a.ADDI(2, 0, 0)                  # r2 = bit counter. Safe to clobber
                                  # unconditionally: BYTE_LOOP's DMEM
                                  # write pointer lives in r6 (see the
                                  # bug-fix note above), specifically so
                                  # RECV_BYTE is free to use r2 however
                                  # it wants without the caller needing
                                  # to save/restore anything.
a.label("BIT_LOOP")

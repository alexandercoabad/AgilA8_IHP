...
module tb_diag_full;
    ...
    // FULL_REGRESSION_WORDS from test.py
    localparam NW = 92;
    reg [15:0] words [0:NW-1];
    initial begin
    ...
    initial begin
        ...
        for (i = NW*2; i < 512; i = i + 1) fmem[i] = 8'h00;
    ...
    always @(posedge clk) begin
        cyc = cyc + 1;
        if (dut.core.pc !== last_pc || dut.core.state !== last_state) begin
            $display("cyc=%0d pc=0x%04x state=%0d flash_mode=%b",
                cyc, dut.core.pc, dut.core.state, dut.flash_mode_r);
            last_pc = dut.core.pc;
    ...
    reg applied_gpio = 0;
    always @(posedge clk) begin
        if (dut.flash_mode_r && !applied_gpio) begin
            ui_in <= 8'h55;
            applied_gpio <= 1;
        end
    end
